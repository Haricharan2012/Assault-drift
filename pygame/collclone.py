...
lbscreen=pygame.image.load("lbimg.png")
...
def sql_connect():
        ...
        # Create a table to store player data if it doesn't already exist
        cursor.execute('''CREATE TABLE IF NOT EXISTS players (id INTEGER PRIMARY KEY,name TEXT,score INTEGER,coins INTEGER)''')
        ...
        df = pd.read_sql_query("SELECT * from players ORDER BY score", conn)
        print(df.head())
        def get_player_one():
...
#---------------------------------------------------------
    ...
    if(charrect.colliderect(lrect)):  #enemy & rightrect
        ...
        ngov=pygame.transform.scale(gov,new_size)  #scale gameover screen
        game_fail=True
...
##                cypos=+400
            ...
            if(collision):                                   #enemy collision
                ...
                speed=speed+0.5                             #increment speed value after getting shot to increase difficulty
...
#changing levels--------------------------------------------------------------------------------
    if level_change:
        ...
        lv2rect=pygame.Rect(lv2rect.width,lv2rect.height,240,150)
...
#------------------------------------------------------------------------------------------------      
    ...
    screen.blit(ngov,(0,0))  #gameover screen
