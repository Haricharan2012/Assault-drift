...
def play_song(song_name: StringVar, song_list: Listbox, status: StringVar):
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS selected_songs (
                            song_selected TEXT
                        )''')

    # Insert the selected song into the table
    cursor.execute("INSERT INTO selected_songs (song_selected) VALUES (?)", (name,))
    conn.commit()

    df = pd.read_sql_query("SELECT * from selected_songs ", conn)
    print(df.head())
